package com.example.ForestFirePrediction.Security.service;

import com.auth0.jwt.JWT;
import com.auth0.jwt.JWTVerifier;
import com.auth0.jwt.algorithms.Algorithm;
import com.auth0.jwt.interfaces.DecodedJWT;
import com.example.ForestFirePrediction.Security.filter.CustomUserDetails;
...
import com.example.ForestFirePrediction.SplData.repo.FireCauseRepo;
import com.example.ForestFirePrediction.SplData.repo.NWCGReportingRepo;
import com.example.ForestFirePrediction.Security.repo.RoleRepo;
import com.example.ForestFirePrediction.Security.repo.UserRepo;
...
//    ---------------------------------------------- USER --------------------------------------------------------------
   ...
   @Override
    ...
    @Override
    public AppUser saveUser(String username, String password, String designation, String pfp, List<String> roles) throws IOException {
        Optional<AppUser> userByUsername = Optional.ofNullable(userRepo.findUserByUsername(username));
        if(userByUsername.isPresent()){
            ...
            AppUser user = new AppUser(null, username, password, designation, pfp, new ArrayList<>());
            user.setUsername(username);
            user.setPassword(passwordEncoder.encode(password));
            if(pfp.isEmpty()){
                user.setPfp("https://via.placeholder.com/150");
            }else{
    ...
    public void updateUser(String username, String designation, String pfp, List<String> roles) throws IOException {
        Optional<AppUser> userByUsername = Optional.ofNullable(userRepo.findUserByUsername(username));
        if(userByUsername.isPresent()){
            ...
            if(pfp.isEmpty()){
                appUser.setPfp("https://via.placeholder.com/150");
            }else{
                if(new Utility().getUrlStatus(pfp) != 404){
                    if(new Utility().checkImage(pfp)){
